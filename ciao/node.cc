...
Node* Node::_error_use(Middleware& m) {
    std::string action_id = gen_random_str();
    ActionHolder action(action_id, this, "error_middleware", m);
    error_middlewares.emplace_back(action);
    return this;
}

Node* Node::error_use(Middleware m) { return _error_use(m); }

Node* Node::error_use(std::vector<Middleware> ms) {
    for (auto& m : ms) {
        _error_use(m);
    }
    return this;
